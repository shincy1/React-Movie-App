...
export default function MovieModal(){
    ...
    const [userRating, setUserRating] = React.useState(0);//test
    ...
    const openInNewTab = url => { // untuk trailer
        window.open(url, '_blank', 'noopener,noreferrer');
    };

    function getTrailer(){ /// fungsi agar jika klik detail video yang ada trailernya maka akan langsung di new tab
    ...
    function addRating(rating) { // fungsi tambah rating dan juga picu konfirmasi
        API.post(`/movie/${movieId}/rating`, {
            value: rating
        })
            .then(response => {
                console.log("Rating added successfully: ", response.data);
                setNotificationMessage("Rating telah ditambahkan! ");
                setShowNotification(true);
                handleNotification("Rating telah ditambahkan! "); 
            })
            .catch(error => {
    ...
    return(
        ...
        >
            <div className="flex gap-2 absolute right-3 top-3">
                    <div>
                        {/* Komponen MovieModal Anda */}

                        {showNotification && (
                            <Notification message={notificationMessage} handleClose={closeNotification} />
                        )}
                    </div>
                <motion.div
                        ...
                        <div className="p-6 flex flex-col ">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex items-center gap-2 ">
                                        <h4 className="text-sm text-neutral-300"><strong>Berikan Rating:</strong></h4>
                                        {[2, 4, 6, 8, 10].map(rating => (
                                            <AiFillStar
                                                key={rating}
                                                size={18}
                                                color={userRating >= rating ? "#FFD700" : "#C4C4C4"}
                                                className="cursor-pointer"
                                                onClick={() => setUserRating(rating)}
                                            />
                                        ))}
                                        <button
                                            className="text-sm text-neutral-300 ml-2 px-3 py-1 bg-neutral-700 rounded-md hover:bg-neutral-600"
